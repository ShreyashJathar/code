...
# Your Bot Token
BOT_TOKEN = "8603545111:AAEzEkr-BYGyrWe5H6slycIqSm_7JIcl6Z0"

# Indian Shortner API Key
API_KEY = "d3a6c313d893910a1b4860b284826b8a2b56bab1"
SHORTENER_API_URL = "https://indianshortner.com/api"

# ==== FEATURE SETTINGS ====
# Channel joining requirement details
FORCE_JOIN_CHANNEL_ID = "@JatharPatil" # Replace with your actual channel username later (e.g., @MyChannel)
CHANNEL_INVITE_LINK = "https://t.me/JatharPatil" # Link button for users
...
def check_membership(user_id):
    ...
    if FORCE_JOIN_CHANNEL_ID == "@JatharPatil" or FORCE_JOIN_CHANNEL_ID == "":
...
def shorten_link(long_url):
    try:
        # Standard API for AdLinkFly-based shorteners (which indianshortner.com is)
        api_url = f"https://indianshortner.com/api?api={API_KEY}&url={long_url}"
...
def handle_movie_upload(message):
    # Security: Verify that the sender is our specified admin by checking their username
    username = message.from_user.username
    if not username or username.lower() not in ADMIN_USERNAMES:
        bot.reply_to(
            message, 
            "❌ **Upload Denied!** Only `@JatharPatil` can upload and add files to the database.", 
            parse_mode="Markdown"
        )
        return
